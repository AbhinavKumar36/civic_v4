  ...
  if (proposals.length === 0) {
    return (
      <div className="text-center py-16 space-y-4">
        <p className="text-gray-500">Please select at least 2 proposals to compare.</p>
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      <div className="flex justify-between items-center bg-white p-4 rounded-lg border">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Comparative Development Proposal Matrix</h2>
          <p className="text-xs text-gray-500">Side-by-side multidimensional comparison for objective decision-support</p>
        </div>
      ...
      <Card className="p-6 overflow-x-auto">
        <h3 className="text-base font-bold text-gray-900 mb-4">Factor Matrix Breakdown</h3>
        <table className="min-w-full text-xs text-left border">
          ...
          <tbody className="divide-y divide-gray-200">
            <tr className="bg-indigo-50/50 font-bold">
              ...
              {proposals.map(p => (
                <td key={p._id} className="py-2.5 px-4 font-extrabold text-indigo-700 text-sm">
                  {p.priorityScore || 0}/100 <span className="text-xs text-gray-500 font-normal">(Rank #{p.priorityRank || '-'})</span>
                </td>
              ))}
            </tr>
            <tr>
              <td className="py-2.5 px-4 font-semibold text-gray-700">Estimated Cost</td>
              {proposals.map(p => (
            ...
            <tr>
              <td className="py-2.5 px-4 font-semibold text-gray-700">Target Beneficiaries</td>
              {proposals.map(p => (
            ...
            <tr>
              <td className="py-2.5 px-4 font-semibold text-gray-700">Social Impact Score</td>
              {proposals.map(p => (
            ...
            <tr>
              <td className="py-2.5 px-4 font-semibold text-gray-700">Economic Impact</td>
              {proposals.map(p => (
            ...
            <tr>
              <td className="py-2.5 px-4 font-semibold text-gray-700">Evidence Confidence</td>
              {proposals.map(p => (
            ...
            <tr>
              <td className="py-2.5 px-4 font-semibold text-gray-700">Connected Demands</td>
              {proposals.map(p => (
                <td key={p._id} className="py-2.5 px-4 text-gray-600">
                  {p.relatedDemandIds?.length || 0} citizen demands
                </td>
              ))}
            </tr>
            <tr>
              <td className="py-2.5 px-4 font-semibold text-gray-700">Detailed Inspection</td>
              {proposals.map(p => (
