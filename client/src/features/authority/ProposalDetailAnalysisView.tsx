...
export const ProposalDetailAnalysisView: React.FC<ProposalDetailAnalysisViewProps> = ({ proposalId, onBack }) => {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
      ...
      {/* Header Banner */}
      <Card className="border-l-4 border-l-indigo-600 p-6 bg-white shadow-sm">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-gray-600 text-sm max-w-3xl leading-relaxed">{proposal.description}</p>
          </div>
          ...
          <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-xl border">
            ...
            <div className="text-center px-3 border-r">
              <span className="block text-xs font-semibold text-gray-500 uppercase">Social</span>
              <span className="text-3xl font-extrabold text-emerald-600">{impact?.socialImpactScore || 0}</span>
              <span className="text-xs text-gray-400">/100</span>
            </div>
            <div className="text-center px-3">
              <span className="block text-xs font-semibold text-gray-500 uppercase">Confidence</span>
        ...
        {/* Quick Specs */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-4 border-t text-sm text-gray-600">
          <div>
      ...
      {/* Navigation Tabs */}
      <div className="flex border-b border-gray-200 bg-white rounded-t-lg px-4 gap-2">
        {[
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Card className="p-5 bg-gradient-to-br from-indigo-50/50 to-white border-indigo-100">
              ...
              <p className="text-3xl font-extrabold text-gray-900 mt-2">{traceabilityLineage?.demandsCount || 0}</p>
              <p className="text-xs text-gray-500 mt-1">Directly related citizen complaints registered</p>
            </Card>
            ...
            <Card className="p-5 bg-gradient-to-br from-emerald-50/50 to-white border-emerald-100">
              ...
              <p className="text-3xl font-extrabold text-gray-900 mt-2">{traceabilityLineage?.themesCount || 0}</p>
              <p className="text-xs text-gray-500 mt-1">Clustered semantic recurring patterns</p>
            </Card>
            ...
            <Card className="p-5 bg-gradient-to-br from-amber-50/50 to-white border-amber-100">
              ...
              <p className="text-3xl font-extrabold text-gray-900 mt-2">{traceabilityLineage?.hotspotsCount || 0}</p>
              <p className="text-xs text-gray-500 mt-1">Geographic high-density clusters</p>
            </Card>
          </div>
            ...
            {proposal.relatedDemandIds && proposal.relatedDemandIds.length > 0 ? (
              <div className="space-y-3">
                {proposal.relatedDemandIds.map((demand: any) => (
                  <div key={demand._id || demand} className="p-4 rounded-lg bg-gray-50 border border-gray-100">
                    ...
                    <p className="text-xs text-gray-600 mt-1">{demand.demandStatement || demand.summary}</p>
                  </div>
                ))}
              </div>
            ) : (
              <p className="text-sm text-gray-500">No direct single demands linked. Linked by category and municipal spatial radius.</p>
            )}
          </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="p-5">
              <h3 className="text-sm font-bold uppercase text-gray-500 mb-3">Target Demographic Cohorts</h3>
            ...
            <Card className="p-5">
              <h3 className="text-sm font-bold uppercase text-gray-500 mb-3">Project Execution Dependencies</h3>
              <div className="flex flex-wrap gap-2">
      ...
      {activeTab === 'priority' && (
        <div className="space-y-6">
          ...
          <Card className="p-6 overflow-x-auto">
            <h3 className="text-base font-bold text-gray-900 mb-3">Auditable Priority Factors Formula</h3>
            <table className="min-w-full text-xs text-left border">
              ...
              <tbody className="divide-y divide-gray-200">
                {priority?.factors && Object.entries(priority.factors).map(([key, f]: [string, any]) => (
                  <tr key={key} className="hover:bg-gray-50">
                    ...
                    <td className="py-2.5 px-3 font-mono text-gray-600">{String(f.rawValue)}</td>
                    ...
                    <td className="py-2.5 px-3 text-gray-500 italic max-w-xs truncate">{f.explanation || 'Calculated via deterministic pipeline'}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot className="bg-gray-50 font-bold border-t">
                <tr>
                  ...
                  <td className="py-3 px-3 text-xs text-gray-500">Sum of (Normalized × Weight)</td>
                </tr>
              </tfoot>
            </table>
          </Card>
      ...
      {activeTab === 'evidence' && (
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-lg font-bold text-gray-900">Documented Public Evidence Corroboration</h3>
          ...
          {priority?.evidenceIds && priority.evidenceIds.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {priority.evidenceIds.map((ev: any) => {
                ...
                return (
                  <Card key={ev._id} className="p-5 border bg-white shadow-sm flex flex-col justify-between">
                    <div>
                      <div className="flex justify-between items-start mb-2">
                        <span className="font-bold text-sm text-gray-900">{ev.indicator}</span>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded ${
                      ...
                      <p className="text-xs font-semibold text-gray-700 mb-1">{ev.relationship}</p>
                      <p className="text-xs text-gray-500 leading-relaxed">{ev.explanation}</p>
                    </div>
      ...
      {activeTab === 'impact' && (
        <div className="space-y-6">
            ...
            <Card className="p-6 bg-gradient-to-br from-emerald-50/40 to-white border-emerald-200">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Multi-criteria citizen welfare & equity enhancement</p>
                </div>
            ...
            <Card className="p-6 bg-gradient-to-br from-blue-50/40 to-white border-blue-200">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <p className="text-xs text-gray-500">Evidence-grounded productivity & efficiency (Zero false precision)</p>
                </div>
                <div className="text-right">
                ...
                {impact?.economicFactors && Object.entries(impact.economicFactors).map(([key, f]: [string, any]) => (
                  <div key={key} className="flex justify-between items-center text-xs">
                    ...
                    {f.status === 'CALCULATED' ? (
                      ...
                      <span className="text-gray-400 font-mono text-[10px] bg-gray-50 px-1.5 py-0.5 rounded">
                        NOT_AVAILABLE
                      </span>
                    )}
                  </div>
          ...
          {/* Scenario Estimates */}
          <Card className="p-6">
            <h3 className="text-base font-bold text-gray-900 mb-4">Catchment Scenario Projections</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 rounded-lg bg-gray-50 border border-gray-200">
                <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Conservative Scenario</span>
                <p className="text-2xl font-black text-gray-800 mt-2">
                  ~{impact?.scenarios?.conservative?.beneficiaries?.toLocaleString() || 'N/A'}
                </p>
                <p className="text-xs text-gray-600 mt-1">{impact?.scenarios?.conservative?.description}</p>
                <span className="inline-block mt-3 text-[10px] font-bold px-2 py-0.5 bg-gray-200 text-gray-700 rounded">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Card className="p-5">
              <div className="flex justify-between items-center mb-3">
                <h3 className="text-sm font-bold uppercase text-gray-700">Uncertainty Drivers</h3>
            ...
            <Card className="p-5">
              <h3 className="text-sm font-bold uppercase text-gray-700 mb-3">Key Model Assumptions</h3>
              <ul className="space-y-2 text-xs text-gray-600">
      ...
      {activeTab === 'traceability' && (
        <div className="space-y-6">
          <Card className="p-6">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-6 bg-gray-50 rounded-xl border">
              <div className="text-center p-3 bg-white rounded-lg border shadow-xs w-full md:w-44">
